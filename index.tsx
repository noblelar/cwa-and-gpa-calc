import { Module, Semester } from "./types";

export const modules: Module[] = [
  {
    course_id: 1,
    course_code: "RN 141",
    course_name: "Basic French I",
    prog_id: "1",
    c_hours: "1",
    inst_id: "1",
    period: "a",
  },
  {
    course_id: 2,
    course_code: "RN 151",
    course_name: "Applied Electricity",
    prog_id: "1",
    c_hours: "3",
    inst_id: "1",
    period: "a",
  },
  {
    course_id: 8,
    course_code: "RN 155",
    course_name: "Technical Drawing",
    prog_id: "1",
    c_hours: "2",
    inst_id: "1",
    period: "a",
  },
  {
    course_id: 9,
    course_code: "RN 157",
    course_name: "Communication Skill I",
    prog_id: "1",
    c_hours: "2",
    inst_id: "1",
    period: "a",
  },
  {
    course_id: 10,
    course_code: "RN 159",
    course_name: "Introduction to Computing",
    prog_id: "1",
    c_hours: "2",
    inst_id: "1",
    period: "a",
  },
  {
    course_id: 11,
    course_code: "RN 167",
    course_name: "Basic Mechanics ",
    prog_id: "1",
    c_hours: "3",
    inst_id: "1",
    period: "a",
  },
  {
    course_id: 12,
    course_code: "RN 169",
    course_name: "Linear Algebra",
    prog_id: "1",
    c_hours: "3",
    inst_id: "1",
    period: "a",
  },
  {
    course_id: 13,
    course_code: "RN 171",
    course_name: "Introduction to Green Chemistry",
    prog_id: "1",
    c_hours: "3",
    inst_id: "1",
    period: "a",
  },
  {
    course_id: 14,
    course_code: "RN 142",
    course_name: "Basic French II",
    prog_id: "1",
    c_hours: "1",
    inst_id: "1",
    period: "b",
  },
  {
    course_id: 15,
    course_code: "RN 152",
    course_name: "Strength of Materials",
    prog_id: "1",
    c_hours: "3",
    inst_id: "1",
    period: "b",
  },
  {
    course_id: 18,
    course_code: "RN 156",
    course_name: "Engineering Drawing",
    prog_id: "1",
    c_hours: "3",
    inst_id: "1",
    period: "b",
  },
  {
    course_id: 19,
    course_code: "RN 158",
    course_name: "Communication Skills II",
    prog_id: "1",
    c_hours: "2",
    inst_id: "1",
    period: "b",
  },
  {
    course_id: 20,
    course_code: "RN 164",
    course_name: "Basic Electronics",
    prog_id: "1",
    c_hours: "3",
    inst_id: "1",
    period: "b",
  },
  {
    course_id: 21,
    course_code: "RN 166",
    course_name: "Calculus",
    prog_id: "1",
    c_hours: "3",
    inst_id: "1",
    period: "b",
  },
  {
    course_id: 22,
    course_code: "RN 168",
    course_name: "Basic Material Science",
    prog_id: "1",
    c_hours: "2",
    inst_id: "1",
    period: "b",
  },
  {
    course_id: 23,
    course_code: "RN 170",
    course_name: "Introduction to Engineering Design",
    prog_id: "1",
    c_hours: "2",
    inst_id: "1",
    period: "b",
  },
  {
    course_id: 33,
    course_code: "RN 251",
    course_name: "Literature in English I",
    prog_id: "1",
    c_hours: "1",
    inst_id: "1",
    period: "c",
  },
  {
    course_id: 34,
    course_code: "RN 265",
    course_name: "Differential Equations",
    prog_id: "1",
    c_hours: "3",
    inst_id: "1",
    period: "c",
  },
  {
    course_id: 35,
    course_code: "RN 267",
    course_name: "Thermodynamics",
    prog_id: "1",
    c_hours: "3",
    inst_id: "1",
    period: "c",
  },
  {
    course_id: 36,
    course_code: "RN 273",
    course_name: "Climate Change and Carbon Balancing",
    prog_id: "1",
    c_hours: "3",
    inst_id: "1",
    period: "c",
  },
  {
    course_id: 37,
    course_code: "RN 275",
    course_name: "Electronics and Instrumentation",
    prog_id: "1",
    c_hours: "3",
    inst_id: "1",
    period: "c",
  },
  {
    course_id: 38,
    course_code: "RN 277",
    course_name: "Solar Cells and Solar Photovoltaic Systems",
    prog_id: "1",
    c_hours: "3",
    inst_id: "1",
    period: "c",
  },
  {
    course_id: 39,
    course_code: "RN 279",
    course_name: "Principles of Bio-energy",
    prog_id: "1",
    c_hours: "3",
    inst_id: "1",
    period: "c",
  },
  {
    course_id: 40,
    course_code: "RN 252",
    course_name: "Literature in English II",
    prog_id: "1",
    c_hours: "1",
    inst_id: "1",
    period: "d",
  },
  {
    course_id: 41,
    course_code: "RN 260",
    course_name: "Mathematical Analysis",
    prog_id: "1",
    c_hours: "3",
    inst_id: "1",
    period: "d",
  },
  {
    course_id: 42,
    course_code: "RN 262",
    course_name: "Computer Application",
    prog_id: "1",
    c_hours: "2",
    inst_id: "1",
    period: "d",
  },
  {
    course_id: 43,
    course_code: "RN 264",
    course_name: "Fluid Mechanics",
    prog_id: "1",
    c_hours: "3",
    inst_id: "1",
    period: "d",
  },
  {
    course_id: 44,
    course_code: "RN 270",
    course_name: "Field Trip and Technical Report Writing I",
    prog_id: "1",
    c_hours: "1",
    inst_id: "1",
    period: "d",
  },
  {
    course_id: 45,
    course_code: "RN 272",
    course_name: "Energy Storage and Distribution Systems",
    prog_id: "1",
    c_hours: "3",
    inst_id: "1",
    period: "d",
  },
  {
    course_id: 46,
    course_code: "RN 274",
    course_name: "Electrochemical Power Sources",
    prog_id: "1",
    c_hours: "3",
    inst_id: "1",
    period: "d",
  },
  {
    course_id: 47,
    course_code: "RN 276",
    course_name: "Wind Energy and Measurement Techniques",
    prog_id: "1",
    c_hours: "3",
    inst_id: "1",
    period: "d",
  },
  {
    course_id: 48,
    course_code: "RN 361",
    course_name: "Probability and Statistics",
    prog_id: "1",
    c_hours: "2",
    inst_id: "1",
    period: "e",
  },
  {
    course_id: 49,
    course_code: "RN 363",
    course_name: "Numerical Analysis",
    prog_id: "1",
    c_hours: "2",
    inst_id: "1",
    period: "e",
  },
  {
    course_id: 50,
    course_code: "RN 365",
    course_name: "Environmental Management",
    prog_id: "1",
    c_hours: "3",
    inst_id: "1",
    period: "e",
  },
  {
    course_id: 51,
    course_code: "RN 371",
    course_name: "Power Generation and Supply",
    prog_id: "1",
    c_hours: "2",
    inst_id: "1",
    period: "e",
  },
  {
    course_id: 52,
    course_code: "RN 373",
    course_name: "Application of Bio-energy",
    prog_id: "1",
    c_hours: "3",
    inst_id: "1",
    period: "e",
  },
  {
    course_id: 53,
    course_code: "RN 375",
    course_name: "Geothermal Energy",
    prog_id: "1",
    c_hours: "3",
    inst_id: "1",
    period: "e",
  },
  {
    course_id: 54,
    course_code: "RN 377",
    course_name: "Solar Energy Collectors ",
    prog_id: "1",
    c_hours: "3",
    inst_id: "1",
    period: "e",
  },
  {
    course_id: 55,
    course_code: "RN 379",
    course_name: "Process Engineering Principles",
    prog_id: "1",
    c_hours: "2",
    inst_id: "1",
    period: "e",
  },
  {
    course_id: 56,
    course_code: "RN 352",
    course_name: "Public Relations",
    prog_id: "1",
    c_hours: "2",
    inst_id: "1",
    period: "f",
  },
  {
    course_id: 57,
    course_code: "RN 356",
    course_name: "Fieldtrip and Technical report Writing II",
    prog_id: "1",
    c_hours: "1",
    inst_id: "1",
    period: "f",
  },
  {
    course_id: 58,
    course_code: "RN 358",
    course_name: "Heat Transfer",
    prog_id: "1",
    c_hours: "3",
    inst_id: "1",
    period: "f",
  },
  {
    course_id: 59,
    course_code: "RN 372",
    course_name: "Energy Efficiency and Conservation",
    prog_id: "1",
    c_hours: "3",
    inst_id: "1",
    period: "f",
  },
  {
    course_id: 60,
    course_code: "RN 374",
    course_name: "Applications of Solar Energy",
    prog_id: "1",
    c_hours: "3",
    inst_id: "1",
    period: "f",
  },
  {
    course_id: 61,
    course_code: "RN 376",
    course_name: "Electrical Machines",
    prog_id: "1",
    c_hours: "3",
    inst_id: "1",
    period: "f",
  },
  {
    course_id: 62,
    course_code: "RN 378",
    course_name: "Internal Combustion Engines",
    prog_id: "1",
    c_hours: "3",
    inst_id: "1",
    period: "f",
  },
  {
    course_id: 63,
    course_code: "RN 451",
    course_name: "Research Methods and Ethics in Science",
    prog_id: "1",
    c_hours: "2",
    inst_id: "1",
    period: "g",
  },
  {
    course_id: 64,
    course_code: "RN 459",
    course_name: "Operations Research",
    prog_id: "1",
    c_hours: "3",
    inst_id: "1",
    period: "g",
  },
  {
    course_id: 65,
    course_code: "RN 461",
    course_name: "Principles of Economics",
    prog_id: "1",
    c_hours: "2",
    inst_id: "1",
    period: "g",
  },
  {
    course_id: 66,
    course_code: "RN 467",
    course_name: "Instrumentation and Control",
    prog_id: "1",
    c_hours: "3",
    inst_id: "1",
    period: "g",
  },
  {
    course_id: 67,
    course_code: "RN 471",
    course_name: "Hydroelectric and Marine Energy Technology",
    prog_id: "1",
    c_hours: "3",
    inst_id: "1",
    period: "g",
  },
  {
    course_id: 68,
    course_code: "RN 473",
    course_name: "Hydrogen Energy and Fuel Cell Technologies",
    prog_id: "1",
    c_hours: "3",
    inst_id: "1",
    period: "g",
  },
  {
    course_id: 69,
    course_code: "RN 450",
    course_name: "Project Work",
    prog_id: "1",
    c_hours: "4",
    inst_id: "1",
    period: "h",
  },
  {
    course_id: 70,
    course_code: "RN 452",
    course_name: "Colloquium / Seminar",
    prog_id: "1",
    c_hours: "1",
    inst_id: "1",
    period: "h",
  },
  {
    course_id: 71,
    course_code: "RN 454",
    course_name: "Business Entrepreneurship",
    prog_id: "1",
    c_hours: "2",
    inst_id: "1",
    period: "h",
  },
  {
    course_id: 72,
    course_code: "RN 458",
    course_name: "Principles of Management",
    prog_id: "1",
    c_hours: "2",
    inst_id: "1",
    period: "h",
  },
  {
    course_id: 73,
    course_code: "RN 460",
    course_name: "Corporate Social Responsibility",
    prog_id: "1",
    c_hours: "3",
    inst_id: "1",
    period: "h",
  },
  {
    course_id: 74,
    course_code: "RN 462",
    course_name: "Industrial and Maintenance Engineering",
    prog_id: "1",
    c_hours: "3",
    inst_id: "1",
    period: "h",
  },
];

export const semesters: Semester[] = [
  {
    id: 1,
    semester: "a",
    name: "year 1 semester 1",
  },
  {
    id: 2,
    semester: "b",
    name: "year 1 semester 2",
  },
  {
    id: 3,
    semester: "c",
    name: "year 2 semester 1",
  },
  {
    id: 4,
    semester: "d",
    name: "year 2 semester 2",
  },
  {
    id: 5,
    semester: "f",
    name: "year 3 semester 1",
  },
  {
    id: 6,
    semester: "g",
    name: "year 3 semester 2",
  },
  {
    id: 7,
    semester: "h",
    name: "year 4 semester 1",
  },
  {
    id: 8,
    semester: "i",
    name: "year 4 semester 2",
  },
];
